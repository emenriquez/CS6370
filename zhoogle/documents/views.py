...
def RankResults(query, results):
    ...
    query = [word for word in no_punctuation.split() if word not in operators]
    for word in query:
        df = 1 + sum(Word.objects.filter(id=word).values_list('df', flat=True))
        tf_idf[word] = math.log2(num_documents / df ) + 1
        query_vector_length += tf_idf[word]*tf_idf[word]
    
    query_vector_length = math.sqrt(query_vector_length)

    doc_list = list(results.values_list('doc_id', flat=True))
    for doc in doc_list:
        doc_vector_length = Document.objects.get(pk=doc).vector_length
        temp_result = results.filter(doc_id=doc)
        for word in query:
